...
from SkewTplus._thermodynamics import (
    _parcelAnalysis4D,
    _parcelAnalysis3D,
    _parcelAnalysis1D,
    _getLCL,
...
def parcelAnalysis(
    ...
    * [height] 1D)
    ...
    if isinstance(dewPointTemperature, masked_array):
        dewPointTemperature.data[dewPointTemperature.mask] = -9999.0
    ...
    args = (
        pressure * pressureConversionFactor,
...
def liftParcel(
    ...
    parcelTemperature = _liftParcel(
        initialTemperature + temperatureConversionOffset,
...
def moistAscent(
    ...
    pressure, temperature = _moistAscent(
        initialPressure=initialPressure * pressureConversionFactor,
...
def getLCL(
    startPressure, startTemperature, startDewPointTemperature, hPa=False, celsius=False
):
    """
    ...
    pressure, temperature = _getLCL(
        startPressure * pressureConversionFactor,
        startTemperature,
...
def barometric_equation(presb_pa, tempb_k, deltah_m, Gamma=-0.0065):
    ...
    return presb_pa * (tempb_k / (tempb_k + Gamma * deltah_m)) ** (
        grav * m_a / (Rstar_a * Gamma)
    )
...
def barometric_equation_inv(heightb_m, tempb_k, presb_pa, prest_pa, Gamma=-0.0065):
    ...
    return (
        heightb_m
        + tempb_k
        * ((presb_pa / prest_pa) ** (Rstar_a * Gamma / (grav * m_a)) - 1)
        / Gamma
    )
...
def ThetaE_Bolton(tempk, pres, e, pref=100000.0):
    ...
    TL = 56 + ((Td - 56.0) ** -1 + (log(T / Td) / 800.0)) ** (-1)
    ...
    thetaE = thetaL * exp((3036.0 / TL - 0.78) * qv * (1 + 0.448 * qv))
...
def GammaW(tempk, pres):
    ...
    A = -1.0 * (1.0 + latent * ws / (Rs_da * tempk))
...
def virtualTemp4(T, p):
    ...
    return (
        (T + degCtoK)
        * (1.0 + (_vaporMixingRatio / Epsilon))
        / (1.0 + _vaporMixingRatio)
    ) - degCtoK
...
def virtualTemp3(T, Td, p):
    ...
    Tc = T + degCtoK
    factor = (1.0 + (_vaporMixingRatio.data / Epsilon)) / (1.0 + _vaporMixingRatio.data)
...
def virtualTemp2(T, qv, p):
    ...
    return T * (1.0 + (qv / Epsilon)) / (1.0 + qv)
...
def SatVap(dwpt, phase="liquid"):
    ...
    print(
        "WARNING: This function is deprecated, please use VaporPressure()"
...
def WetBulb(tempc, RH):
    ...
    Tw = (
        tempc * arctan(0.151977 * (RH + 8.313659) ** 0.5)
        + arctan(tempc + RH)
        - arctan(RH - 1.676331)
